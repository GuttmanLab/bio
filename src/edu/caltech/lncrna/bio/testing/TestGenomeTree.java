...
public class TestGenomeTree {
    ...
    private Annotated singletonValue = new Annotation("chr1", 500, 1000,
            Strand.POSITIVE);
    ...
    public void setup() {
        ...
        singletonTree.add(singletonValue);
        
        fullTree = new GenomeTree<>();
        try (BedParser bp = new BedParser(BED)) {
            bp.stream().forEach(x -> fullTree.add(x));
        }
    }
    ...
    public void testEmptyTreeNumOverlappersIsZero() {
        assertThat(emptyTree.numOverlappers(singletonValue), is(0));
    }
    
    @Test
    public void testEmptyTreeOverlapperIteratorIsEmpty() {
        assertThat(emptyTree.overlappers(singletonValue).hasNext(), is(false));
    }
    
    @Test
    public void testEmptyTreeOverlapperIteratorNextThrowsException() {
        thrown.expect(NoSuchElementException.class);
        emptyTree.overlappers(singletonValue).next();
    }
    
    @Test
    public void testEmptyTreeAnyGeneBodyOverlapsIsFalse() {
        assertThat(emptyTree.bodyOverlaps(singletonValue), is(false));
    }
    ...
    public void testSingletonTreeNumOverlappersIsOne() {
        assertThat(singletonTree.numOverlappers(singletonValue), is(1));
    }
    
    @Test
    public void testSingletonTreeAnyGeneBodyOverlapsIsTrue() {
        assertThat(singletonTree.bodyOverlaps(singletonValue), is(true));
    }
    ...
    public void testTreeOverlapPositive() {
        ...
        tree.add(new Annotation("chr1", 100, 200, Strand.POSITIVE));
        tree.add(new Annotation("chr1", 300, 400, Strand.POSITIVE));
        Annotated testAnnot1 = new Annotation("chr1", 100, 200,
                Strand.POSITIVE);
        assertThat(tree.overlaps(testAnnot1), is(true));
        assertThat(tree.numOverlappers(testAnnot1), is(1));
        Annotated testAnnot2 = new Annotation("chr1", 150, 350,
                Strand.POSITIVE);
        assertThat(tree.overlaps(testAnnot2), is(true));
        assertThat(tree.numOverlappers(testAnnot2), is(2));
        Annotated testAnnot3 = new Annotation("chr1", 50, 400,
                Strand.POSITIVE);
        assertThat(tree.overlaps(testAnnot3), is(true));
        assertThat(tree.numOverlappers(testAnnot3), is(2));
    }
    ...
    public void testTreeGeneBodyOverlapIntronIsTrue() {
        ...
        tree.add(Annotation.builder()
                ...
                .build());
        Annotated testAnnot1 = new Annotation("chr1", 200, 300, Strand.POSITIVE);
        assertThat(tree.bodyOverlaps(testAnnot1), is(true));
    }
    ...
    public void testFullTreeSize() {
        assertThat(fullTree.size(), is(NUM_BED_RECORDS));
    }
}